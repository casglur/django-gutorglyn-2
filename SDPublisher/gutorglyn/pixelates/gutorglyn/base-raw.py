...
def l(element, context, args):        
    ...
    if context=="begin":
        ...
        if lgType == "CH":
            ...
            if lNum == "1":
                html = '<span class="cywydd-line-number">' + printLineNum(lNum) + '</span><span class="cywydd-line-1">'      
                return html

            #check the number of the current descendent <l> of the <lg> element   
            if lineNumber == 1:
                html = '<span class="cywydd-line-number">' + printLineNum(lNum) + '</span><span class="cywydd-line-first">'         
                return html 
 
            else:
                html = '<span class="cywydd-line-number">' + printLineNum(lNum) + '</span><span class="cywydd-line">'      
                return html                 
...
def figure(element, context, args):
    ...
    if context=="begin":        
        html='<p><a href="/static/img/%s/%s" target="_blank"><img style="border-style:solid; border-color: #A20A1B #3D3D3D #3D3D3D #A20A1B ;" width="90%%" src="/static/img/%s/%s"/></a><br/><i>%s</i>' % (graphicDir,graphicNodeSrc,graphicDir,graphicNodeSrc,descriptionNode)
        return html
    if context=="end":              
        html='</p>' 
        return html
