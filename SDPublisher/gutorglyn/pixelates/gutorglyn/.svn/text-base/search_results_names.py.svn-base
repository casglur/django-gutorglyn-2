...
def p(element, context, args):
    ...
    topDiv = div.get_parent_element()
    nameID = topDiv.get_attribute_value('xml:id')
    head = element.get_previous_sibling_by_name('head')
    html = ""
    try:
        rend = element.get_attribute_value('rend')
    except:
        if context=="begin":
            html = '<a href="/gutorglyn/name-full/?n=%s"><strong>%s</strong></a>&nbsp;<a href="/gutorglyn/name-full/?n=%s"><img alt="goto poem" src="/static/img/icons/goto-icon.png" width="16px" height="16px"></a><br/><p>' % (nameID, str(head), nameID)
        if context=="end":
            html = '</p>'
    else:
        if context=="begin":
...
#     poemTitleValue = poemDiv.get_child('head')
    ...
    html=""
    if context=="begin":
        html = '<a href="/gutorglyn/poem/?poem-selection=' + poemID + '"><strong>' + str(poemTitleValue) + '</strong>&nbsp;<img alt="goto poem" src="/static/img/icons/goto-icon.png" width="16px" height="16px"></a><br/><div class="line-group">' 
        return html
    if context=="end":
        html = '</div><br/>'
        return html
