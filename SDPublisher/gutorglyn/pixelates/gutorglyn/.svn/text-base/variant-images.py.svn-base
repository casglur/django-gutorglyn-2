...
def pb(element, context, args):
    ...
    if llgcRef not in imgIDFull:        
        ...
        if context == "begin":        
#             html = '<br/><a href="#" onclick="return false;" class="manuscript-image-link" id="manuscript-image-%s"><img src="%s"/>' % (imgID,imgSrc)
            html = '<br/><img src="%s"/>' % (imgSrc)
            return html
        
        if context == "end":              
#             html='</a>&nbsp;&nbsp;<a href="%s" target="_blank">%s</a><br/>'  % (imgID,imgRef)
            html='</a>&nbsp;&nbsp;%s<br/>'  % (imgRef)
            return html         
    else:  
        ...
        if imgID == '':            
            ...
            imgThumbHtml = '<img src="/static/img/icons/%s"/>' % imgSrc
        else:
            imgSrc = 'http://dams.llgc.org.uk/behaviour/llgc-id:%s/fedora-bdef:image/thumbnail' % imgID
            imgThumbHtml = '<img src="%s"/>' % imgSrc
        
        if context == "begin":        
            html = '<br/><a href="#" onclick="return false;" class="manuscript-image-link" id="manuscript-image-%s">%s' % (imgID,imgThumbHtml)
            return html
        
        if context=="end":              
            html='</a>&nbsp;&nbsp;<a href="%s" target="_blank">%s</a><br/>'  % (imgLink,imgRef)
            return html           
