...
mymgr = XmlManager()
myquery = "<select id='poem' name='poem'><option value='#'>Dewis...</option> "
myquery = myquery + "{ for $x in collection('../../../gutorglyn.bdbxml')//tei:div[@type='lev0'] order by $x/@xml:id return"
myquery = myquery + "<option value='{substring-after($x/@xml:id, 'Guto')}'>"
myquery = myquery + "{$x/tei:head/text()}"
